...
import{ getFirestore, collection, onSnapshot, query, orderBy, limit, getDocs, doc, getDoc, deleteDoc, serverTimestamp, where, updateDoc, runTransaction, arrayUnion }from 'firebase/firestore';
...
const BattleScreen =({ onExitBattle, battleId, playerId }:{ onExitBattle:(battleId: string) => void; battleId: string; playerId: string; }) =>{
        ...
        const processTurn = async() =>{
            ...
            await runTransaction(db, async(transaction) =>{
                ...
                const executeAttack =(attacker: Player, defender: Player) =>{
                    ...
                    const defenderPoke = defender.team[defender.activePokemonIndex];
                    if(attackerPoke.isFainted || defenderPoke.isFainted) return false;
                    if(!attacker.selectedAction){
                        return false;
                    }
                    if(attacker.selectedAction.type !== 'move'){
                        return false;
                    }
                    actionOccurred = true;
                    const moveIndex = attackerPoke.moves.findIndex(m => m.name === attacker.selectedAction!.payload);
                    if(moveIndex === -1 || attackerPoke.moves[moveIndex].pp === 0){
                        log.push(`${attackerPoke.name}tentou usar um movimento, mas falhou!`);
                        return false;
                    }  
                    ...
                    defender.team[defender.activePokemonIndex] = calculateDamage(attackerPoke, defenderPoke, move, log);
                ...
                if(p1Fainted){
                    ...
                    if(actionOccurred){
                        ...
                        log.push(`--- Turno ${newState.turnNumber}---`);
                    }
                }
                newState.log = log;
    ...
    const handleAction = async(action: TentativeAction) =>{
        ...
        if(battleState.turn.includes('_must_switch')){
            if(action.type === 'switch'){
                ...
                updateData.log = arrayUnion(`${battleState[playerKey].name}trocou para ${battleState[playerKey].team[action.payload].name}!`, `--- Turno ${battleState.turnNumber}---`);
    ...
    const handleSendMessage = async() =>{
        if(!chatMessage.trim() || !battleState) return;
                ...
                <View style={styles.controlsContainer}>
                    ...
                    <View style={styles.rightPanel}>
                        <View style={styles.tabSelector}>
                            <TouchableOpacity onPress={() => setActiveTab('log')}style={[styles.tabButton, activeTab === 'log' && styles.activeTabButton]}>
                                <Text style={styles.tabButtonText}>Log</Text>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => setActiveTab('chat')}style={[styles.tabButton, activeTab === 'chat' && styles.activeTabButton]}>
                                <Text style={styles.tabButtonText}>Chat</Text>
                            </TouchableOpacity>
                        </View>
                        <View style={styles.tabContent}>
                           {activeTab === 'log' ?(
                                <ScrollView ref={logScrollViewRef}onContentSizeChange={() => logScrollViewRef.current?.scrollToEnd({ animated: true })}>
                                   {battleState.log.slice(-25).map((msg, index) => <Text key={index}style={[styles.logText, msg.startsWith('---') && styles.turnMarkerText]}>{msg}</Text>)}
                                </ScrollView>
                            ) :(
                                <View style={styles.chatContainer}>
                                    <ScrollView ref={chatScrollViewRef}onContentSizeChange={() => chatScrollViewRef.current?.scrollToEnd({ animated: true })}style={styles.chatMessages}>
                                       {battleState.chat.sort((a,b) =>(a.timestamp?.seconds ?? 0) -(b.timestamp?.seconds ?? 0)).map((msg, index) =>(
                                            <View key={index}style={[styles.chatMessageBubble, msg.senderId === playerId ? styles.myChatMessage : styles.opponentChatMessage]}>
                ...
                await runTransaction(db, async(transaction) =>{
                    ...
                    if(!querySnapshot.empty && querySnapshot.docs[0].id !== id){
                        ...
                        const newBattleState: BattleState ={
                            ...
                            turn: 'selecting',
                            turnNumber: 1,
                            log: [`--- Turno 1 ---`, `A batalha entre ${opponentData.name}e ${name}começou!`],
                            chat: [],
                            state: 'ongoing',
...
const styles = StyleSheet.create({
    ...
    rightPanel:{ flex: 1, flexDirection: 'column', backgroundColor: 'rgba(0,0,0,0.2)' },
    tabSelector:{ flexDirection: 'row', borderBottomWidth: 1, borderColor: '#444' },
    tabButton:{ flex: 1, paddingVertical: 10, alignItems: 'center', backgroundColor: '#333' },
    activeTabButton:{ backgroundColor: '#555', borderBottomWidth: 2, borderColor: '#FFCB05' },
    ...
    turnMarkerText:{ color: '#FFCB05', fontWeight: 'bold', textAlign: 'center' },
    chatContainer:{ flex: 1 },
    chatMessages:{ flex: 1 },
    chatMessageBubble:{ borderRadius: 10, padding: 8, marginVertical: 4, maxWidth: '85%' },
    myChatMessage:{ backgroundColor: '#0561FC', alignSelf: 'flex-end' },
    opponentChatMessage:{ backgroundColor: '#333', alignSelf: 'flex-start' },
    chatSenderName:{ color: '#ccc', fontSize: 10, fontWeight: 'bold', marginBottom: 2 },
    chatMessageText:{ color: 'white', fontSize: 14 },
    chatInputContainer:{ flexDirection: 'row', alignItems: 'center', borderTopWidth: 1, borderColor: '#555', paddingTop: 8, marginTop: 4 },
    chatInput:{ flex: 1, backgroundColor: '#222', color: 'white', borderRadius: 20, paddingHorizontal: 15, paddingVertical: 8, marginRight: 8 },
    sendButton:{ backgroundColor: '#2196F3', borderRadius: 20, padding: 10 },
